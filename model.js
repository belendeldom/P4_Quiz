...
* pero al final del módulo se llama a load() para la carga las preguntas
...
exports.update = (id, question, answer) => {
	...
	const quiz = quizzes[id];   // para ver si el identificador es correcto.
	if (typeof quiz === "undefined"){
		throw new Error(`El valor del parámetro id no es válido.`);
	}
	quizzes.splice(id, 1, {   // quito el elemento que haya en id y modifico 1.
		question: (question || "").trim(),
		answer: (answer || "").trim()
	});
	save();
...
* @returns (any)
*/
exports.getAll = () => JSON.parse(JSON.stringify(quizzes)); //parseo el array de quizzes porque si.
